...
const STATUS_OPTIONS = ["Todo", "Research", "Script", "Filming", "Posted", "Checked"];
const VIDEO_TYPES = ["Full Preview", "Quick Pick", "Key Battles", "Daily Picks", "Recap"];
...
const BIG_TEAMS = new Set(["Brazil", "Argentina", "England", "France", "Germany", "Spain", "Portugal", "Netherlands", "Belgium", "Uruguay"]);
const TIER_B_TEAMS = new Set([
  "Mexico", "USA", "Canada", "Morocco", "Croatia", "Switzerland", "Colombia", "Japan",
  "Sweden", "Ecuador", "Australia", "South Korea", "Scotland", "Türkiye", "Senegal",
  "Norway", "Austria", "Algeria", "Ghana", "Ivory Coast", "Paraguay", "Czechia"
]);

const RAW_MATCHES = [
  ["2026-06-12", "02:00", "Mexico", "South Africa", "Bảng A", "Mexico City"],
  ["2026-06-12", "09:00", "South Korea", "Czechia", "Bảng A", "Guadalajara"],
  ["2026-06-13", "02:00", "Canada", "Bosnia and Herzegovina", "Bảng B", "Toronto"],
  ["2026-06-13", "08:00", "USA", "Paraguay", "Bảng D", "Los Angeles"],
  ["2026-06-13", "11:00", "Qatar", "Switzerland", "Bảng B", "San Francisco Bay Area"],
  ...
  ["2026-06-17", "11:00", "Austria", "Jordan", "Bảng J", "San Francisco Bay Area"],
  ...
  ["2026-06-18", "23:00", "Switzerland", "Bosnia and Herzegovina", "Bảng B", "Los Angeles"],
  ["2026-06-19", "05:00", "Canada", "Qatar", "Bảng B", "Vancouver"],
  ["2026-06-19", "08:00", "Mexico", "South Korea", "Bảng A", "Guadalajara"],
  ["2026-06-19", "11:00", "Türkiye", "Paraguay", "Bảng D", "San Francisco Bay Area"],
  ...
  ["2026-06-23", "10:00", "Jordan", "Algeria", "Bảng J", "San Francisco Bay Area"],
  ...
  ["2026-06-25", "02:00", "Bosnia and Herzegovina", "Qatar", "Bảng B", "Seattle"],
  ...
  ["2026-06-26", "03:00", "Ecuador", "Germany", "Bảng E", "New York/New Jersey"],
  ...
  ["2026-06-26", "09:00", "Paraguay", "Australia", "Bảng D", "San Francisco Bay Area"],
...
const DEFAULT_MATCHES = RAW_MATCHES.map((row, index) => buildDefaultMatch(row, index));
...
function buildDefaultMatch(row, index) {
  ...
  return {
    ...
    publishTime,
    predictedScore,
    notes: tier === "A" ? "Ưu tiên bản kế hoạch sâu, câu mở đầu mạnh và đăng sớm." : tier === "B" ? "Theo dõi đội hình, có thể làm dự đoán nhanh." : "Nội dung ngắn, tập trung góc nhìn dễ hiểu.",
    ...
    tacticalNotes: "Ghi lại pressing, transition, set-piece và điểm yếu phòng ngự hai đội.",
    ...
    scriptOutline: makeScriptTemplate({ homeTeam, awayTeam, group, predictedScore }),
    ...
    hashtags: "#WorldCup2026 #FootballPreview #TikTokFootball #ReelsFootball #Shorts",
...
function cacheElements() {
  [
    "heroDate", "heroProgressText", "heroProgressBar", "dateFilter", "groupFilter", "teamFilter",
    "tierFilter", "statusFilter", "videoTypeFilter", "resetBtn", "exportBtn", "importBtn", "importFile",
    "statTotal", "statTodo", "statWorking", "statPosted", "statTierA", "listTitle", "resultCount",
    "matchesList", "emptyState", "modalOverlay", "closeModalBtn", "modalMeta", "modalTitle",
    "modalSubtitle", "matchForm", "toast"
  ].forEach((id) => {
...
function populateControls() {
  fillSelect(els.dateFilter, getDates(state.matches).map((date) => [date, formatDate(date)]), state.selectedDate);
  fillSelect(els.groupFilter, [["all", "Tất cả bảng"], ...unique(state.matches.map((m) => m.group)).map((g) => [g, g])], "all");
  fillSelect(els.teamFilter, [["all", "Tất cả đội"], ...getTeams(state.matches).map((team) => [team, team])], "all");
  fillSelect(els.tierFilter, [["all", "Tất cả tier"], ...TIER_OPTIONS.map((tier) => [tier, `Tier ${tier}`])], "all");
  fillSelect(els.statusFilter, [["all", "Tất cả trạng thái"], ...STATUS_OPTIONS.map((status) => [status, labelStatus(status)])], "all");
  fillSelect(els.videoTypeFilter, [["all", "Tất cả loại video"], ...VIDEO_TYPES.map((type) => [type, labelVideoType(type)])], "all");

  fillSelect(els.matchForm.elements.tier, TIER_OPTIONS.map((tier) => [tier, `Tier ${tier}`]));
  fillSelect(els.matchForm.elements.videoType, VIDEO_TYPES.map((type) => [type, labelVideoType(type)]));
  fillSelect(els.matchForm.elements.status, STATUS_OPTIONS.map((status) => [status, labelStatus(status)]));
  fillSelect(els.matchForm.elements.confidenceLevel, CONFIDENCE_OPTIONS.map((value) => [value, labelConfidence(value)]));
  fillSelect(els.matchForm.elements.predictionCheck, CHECK_OPTIONS.map((value) => [value, labelCheck(value)]));
}

function fillSelect(select, entries, value) {
  select.innerHTML = entries.map(([optionValue, label]) => `<option value="${escapeAttr(optionValue)}">${escapeHtml(label)}</option>`).join("");
...
function render() {
  const dayMatches = state.matches.filter((match) => match.vnDate === state.selectedDate);
...
function renderHero(dayMatches) {
  const posted = dayMatches.filter((m) => ["Posted", "Checked"].includes(m.status)).length;
...
function renderStats(dayMatches) {
  ...
  els.statTodo.textContent = dayMatches.filter((m) => m.status === "Todo").length;
  els.statWorking.textContent = dayMatches.filter((m) => ["Research", "Script"].includes(m.status)).length;
  els.statPosted.textContent = dayMatches.filter((m) => m.status === "Posted").length;
...
function renderMatchCard(match) {
  ...
  const tierColor = match.tier === "A" ? "#ffd166" : match.tier === "B" ? "#48a6ff" : "#d7dce0";
...
function getFilteredMatches() {
  return state.matches
    ...
    .filter((match) => state.filters.team === "all" || match.homeTeam === state.filters.team || match.awayTeam === state.filters.team)
    .filter((match) => state.filters.tier === "all" || match.tier === state.filters.tier)
    .filter((match) => state.filters.status === "all" || match.status === state.filters.status)
    .filter((match) => state.filters.videoType === "all" || match.videoType === state.filters.videoType)
    .sort((a, b) => a.vnTime.localeCompare(b.vnTime) || a.matchNumber - b.matchNumber);
}
...
function copyContent(type) {
  ...
  copyText(contentMap[type] || "").then(() => {
    showToast("Đã chép nội dung vào bộ nhớ tạm.");
  }).catch(() => {
    showToast("Không thể chép tự động. Hãy chọn nội dung trong bản kế hoạch và chép thủ công.");
  });
}
...
function resetData() {
  ...
  state.filters = { group: "all", team: "all", tier: "all", status: "all", videoType: "all" };
  saveMatches();
  populateControls();
  render();
...
function normalizeImportedMatch(match, index) {
  ...
  if (!merged.id || !merged.vnDate || !merged.vnTime || !merged.homeTeam || !merged.awayTeam) {
    throw new Error("Invalid match");
  }
  if (!TIER_OPTIONS.includes(merged.tier)) merged.tier = inferTier(merged.homeTeam, merged.awayTeam);
  if (!STATUS_OPTIONS.includes(merged.status)) merged.status = "Todo";
  if (!VIDEO_TYPES.includes(merged.videoType)) merged.videoType = inferVideoType(merged.tier, index);
  if (!CONFIDENCE_OPTIONS.includes(merged.confidenceLevel)) merged.confidenceLevel = "Medium";
  if (!CHECK_OPTIONS.includes(merged.predictionCheck)) merged.predictionCheck = "";
  return merged;
}
...
function loadMatches() {
  try {
    ...
    if (!Array.isArray(matches) || matches.length === 0) return clone(DEFAULT_MATCHES);
...
function saveMatches() {
  localStorage.setItem(STORAGE_KEY, JSON.stringify({ version: 1, matches: state.matches }));
}
...
function getTeams(matches) {
  return unique(matches.flatMap((match) => [match.homeTeam, match.awayTeam])).sort((a, b) => a.localeCompare(b));
}
